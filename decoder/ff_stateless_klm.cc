...
string StatelessKLanguageModel<Model>::usage(bool /*param*/,bool /*verbose*/) {
    return "StatelessKLanguageModel";
}
...
bool ParseLMArgs(string const& in, string* filename, string* mapfile, bool* explicit_markers, string* featname) {
  ...
  *featname="StatelessLanguageModel";
  *mapfile = "";
...
#undef LMSPEC_NEXTARG
      default:
      fail:
        cerr<<"Unknown StatelessKLanguageModel option "<<s<<" ; ";
        goto usage;
      }
    } else {
      if (filename->empty())
  ...
  if (!filename->empty())
    return true;
usage:
  cerr << "StatelessKLanguageModel is incorrect!\n";
  return false;
}
...
template <class Model>
class StatelessKLanguageModelImpl {
 public:
  double LookupWords(const TRule& rule, const vector<const void*>& ant_states, double* oovs, double* emit) {
    *oovs = 0;
    *emit = 0;
    const vector<WordID>& e = rule.e();
    BoundaryAnnotatedState dummy;
    BoundaryRuleScore<Model> ruleScore(*ngram_, dummy);
    unsigned i = 0;
    double my_prob = 0.f;
    if (e.size()) {
      if (e[i] == kCDEC_SOS) { // deals with <s>
    ...
    for (; i < e.size(); ++i) {
      if (e[i] <= 0) { // deals with NTs
        my_prob += ruleScore.Finish();
        ruleScore.Reset();
      } else { // deals with terminals
        float ep = 0.f;
 ...
 public:
  StatelessKLanguageModelImpl(const string& filename, const string& mapfile, bool explicit_markers) :
...
template <class Model>
StatelessKLanguageModel<Model>::StatelessKLanguageModel(const string& param) {
    ...
    try {
        pimpl_ = new StatelessKLanguageModelImpl<Model>(filename, mapfile, explicit_markers);
...
template <class Model>
StatelessKLanguageModel<Model>::~StatelessKLanguageModel() {
    delete pimpl_;
}

template <class Model>
void StatelessKLanguageModel<Model>::TraversalFeaturesImpl(const SentenceMetadata& /* smeta */,
    ...
    double oovs = 0;
    double emit = 0;
    features->set_value(fid_, pimpl_->LookupWords(*edge.rule_, ant_states, &oovs, &emit));
    if (oovs && oov_fid_)
...
template <class Model> boost::shared_ptr<FeatureFunction> CreateModel(const std::string &param) {
    StatelessKLanguageModel<Model> *ret = new StatelessKLanguageModel<Model>(param);
    return boost::shared_ptr<FeatureFunction>(ret);
}

boost::shared_ptr<FeatureFunction> StatelessKLanguageModelFactory::Create(std::string param) const {
    ...
    ModelType m;
    if (!RecognizeBinary(filename.c_str(), m)) 
        m = HASH_PROBING;
...
std::string  StatelessKLanguageModelFactory::usage(bool params, bool verbose) const {
    return StatelessKLanguageModel<lm::ngram::Model>::usage(params, verbose);
}
